...
export default function PaymentScreen({ navigation, route }) {
    ...
    navigation.setOptions({
      ...
      headerRight: () => {
        if(Device.osName === 'Android'){
          if(canGoBack){
            return (
              <EditButton onPress={() => navigation.navigate('NeedPaymentDetail', { telemedicineData: telemedicineData })}>
                <Text T5 bold>X</Text>
              </EditButton>
            );
          }
  ...
  function handlePaymentComplete(url) {
    if (url?.split('?invoice_id=')[1]) {
      ...
      navigation.navigate('NeedPaymentDetail', { telemedicineData: telemedicineData })
    }
  }
