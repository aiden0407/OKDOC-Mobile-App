...
export default function RegisterCompleteScreen({ navigation, route }) {
  ...
  const initPatient = async function (loginToken) {
    try {
      const createPatientByBirthResponse = await createPatientByBirth(loginToken, registerStatus.email, name, birth, gender);
    ...
    <SafeArea>
      <Container paddingHorizontal={20}>
        <ContainerCenter>
          ...
          <Text T2 bold marginTop={18}>아래 계정을 확인해주세요</Text>
          <Text T3 medium color={COLOR.GRAY1} marginTop={6}>동일한 정보로 등록된 계정이 존재합니다</Text>
          <Text T6 center color={COLOR.GRAY1} marginTop={30}>아래 계정이 본인이 아니라면,{'\n'}회원가입을 계속 진행해주세요</Text>
